import csv
import tkinter as tk #goat1
from tkinter import *
from tkinter import ttk #goat2
import pandas as pd
import os
...
class Character: #character class to hold character data and create og CSV purrr
    ...
    def __init__(self, name, element, character_star, character_level, max_hp, base_ATK, elemental_skill, elemental_burst, region, weapon_owned):
        ...
        Character.characters.append(self) #for the character list to make sure its added to the intiital csv file
...
# this is for the csv if the  csv called "genshinCharacters.csv" does not exist
characters = [ 
...
#bro again order matters nearly cried 
...
#the hottification of treeview stuff 
        self.style = ttk.Style()
        self.style.theme_use("clam")  #lets u customise moreeee lifesaverrr
        self.style.configure("Treeview", 
                           background= "#222437"  ,
                           foreground= "#7FB1CE",
                           fieldbackground = "#222437",
        ...
        self.page1 = tk.Frame(self.genshin_window, bg = "#7FB1CE") 
        self.page2 = tk.Frame(self.genshin_window, bg = "#7FB1CE" )
        self.page3 = tk.Frame(self.genshin_window, bg = "#7FB1CE")
        ...
        self.page2_title = tk.Label(self.page2, text="SORT CHARACTERS", bg="#7FB1CE", font=("Ariel",30, "bold"))
        self.page2_title.pack(side= TOP)
        self.page2_search= tk.Label(self.page2, text= "You can search for: Name, Element, Star, Region and Weapon ;)",font=("Ariel",14, "bold"), bg="#7FB1CE")
        self.page2_search.pack(side=TOP)
        self.page2_advice= tk.Label(self.page2, text= "Press 'Search' with the search box empty to get all characters",font=("Ariel",14, "bold"), bg="#7FB1CE")
        self.page2_advice.pack(side=TOP)

        self.page3_title = tk.Label(self.page3, text="EDIT CHARACTERS", font=("Ariel",30, "bold"), bg="#7FB1CE" )
        self.page3_title.pack(side= TOP)
        self.info_label= tk.Label(self.page3, text= "To view edits on page 2, close the window and reopen :D",font=("Ariel",14, "bold"), bg="#7FB1CE")
        self.info_label.pack(side=TOP)

        #This is a place holder ish error message for when boxes are empty for certain buttons
        self.error_label_2 = tk.Label(self.page2, text="", font=("Ariel",15, "bold"), bg="#7FB1CE", fg="#222437" )
        self.error_label_2.pack(side=BOTTOM)

        self.error_label_3 = tk.Label(self.page3, text="", font=("Ariel",15, "bold"), bg="#7FB1CE", fg="#222437" )
        self.error_label_3.pack(side=BOTTOM)

        #frame for editting entry boxes
        self.frame_editting = tk.Frame(self.page3, bg="#7FB1CE")
        self.frame_editting.pack(side=BOTTOM)
 
        self.name_label = Label(self.frame_editting, text="Name:", font=20, bg="#7FB1CE" )#name box
        self.name_label.grid(row=0, column=0)
        self.name_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.name_entry.grid(row=1, column=0)
        
        self.element_label = Label(self.frame_editting, text="Element:", font=20, bg="#7FB1CE")#element box
        self.element_label.grid(row=0, column=1)
        self.element_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.element_entry.grid(row=1, column=1)

        self.star_label = Label(self.frame_editting, text="Star:", font=20, bg="#7FB1CE")#star box
        self.star_label.grid(row=0, column=2)
        self.star_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.star_entry.grid(row=1, column=2)

        self.level_label = Label(self.frame_editting, text="Level:", font=20, bg="#7FB1CE")#level box
        self.level_label.grid(row=0, column=3)
        self.level_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.level_entry.grid(row=1, column=3)

        self.hp_label = Label(self.frame_editting, text="HP:", font=20, bg="#7FB1CE")#HP box
        self.hp_label.grid(row=0, column=4)
        self.hp_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.hp_entry.grid(row=1, column=4)

        self.atk_label = Label(self.frame_editting, text="Base ATK:", font=20, bg="#7FB1CE")#aTK box
        self.atk_label.grid(row=0, column=5)
        self.atk_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.atk_entry.grid(row=1, column=5)

        self.skill_label = Label(self.frame_editting, text="Skill:", font=20, bg="#7FB1CE")#skill box
        self.skill_label.grid(row=0, column=6)
        self.skill_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.skill_entry.grid(row=1, column=6)

        self.burst_label = Label(self.frame_editting, text="Burst", font=20, bg="#7FB1CE")#burst box
        self.burst_label.grid(row=0, column=7)
        self.burst_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.burst_entry.grid(row=1, column=7)

        self.region_label = Label(self.frame_editting, text="Region:", font=20, bg="#7FB1CE")#region box
        self.region_label.grid(row=0, column=8)
        self.region_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        self.region_entry.grid(row=1, column=8)

        self.weapon_label = Label(self.frame_editting, text="Weapon", font=20, bg="#7FB1CE") #weapon box
        self.weapon_label.grid(row=0, column=9)
        self.weapon_entry = Entry(self.frame_editting, font=20, width=10, bg="#2c77d8")
        ...
        self.search_entry = Entry(self.page2, width=35, bg="#2c77d8", font=18)
...
class IntroPage(GenshinSorter): #inheritance yurrrrrrrrrrrrrrrr
    def __init__(self, genshin_window):
        ...
        self.page1_title = tk.Label(self.page1, text="HAFSA'S GENSHIN ORGANISER", font=('Arial', 30, "bold"),bg="#7FB1CE")
        self.page1_title.pack(side= TOP, pady=20, expand=True)
        self.theme_label = tk.Label(self.page1, text="Current Theme: Neuvillette", font=('Arial', 16),bg="#7FB1CE")
        self.theme_label.pack(side= BOTTOM, pady=20, expand=True)
        #Character count #extraStats too good
        self.character_count_label = tk.Label(self.page1, text= f"TOTAL CHARACTER COUNT: {Character.character_count}" , font=('Arial', 15, "bold"), bg="#7FB1CE") 
        ...
        fig.patch.set_facecolor("#7FB1CE")  #set the background of the figure to the lightblue ting
        #plots the pie chart
